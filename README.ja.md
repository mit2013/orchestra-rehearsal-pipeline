...
English overview: [README.md](README.md)。設計書は [`docs/`](docs/)、作った経緯は [`docs/journal/`](docs/journal/) にある。
...
共通に**ダイナミクス処理を1回だけ通す。
    ...
    残響 → ゲイン → パラレルコンプ → コンプレッサ(2:1、緩く) → トゥルーピークリミッター(-1 dBTP)

### パラレルコンプ ― 小さい音だけを持ち上げる

ダウンワードのコンプは**大きい音を下げる**ので、静かなところは録れたままの小ささで
出ていく。260829 後半 00:31:32 からの45秒で、1秒ごとの実効音量は静音部 -52.7 dBFS /
大音量部 -13.1 dBFS と **39.5 dB** 開いていた。スマホの内蔵スピーカーでは前者は聴こえない。
車内でも、大きいところをロードノイズに勝つ音量にすると小さいところが埋もれる。

クラシックの録音ではこれをパラレルコンプで解く。信号を2つに分け、片方をピークで大きく
潰れるコンプに通して持ち上げ、直の信号に足し戻す。**大きいところはほぼ変わらず、
小さいところだけが上がる。**

| | 静音部(下位10%) | 大音量部(上位10%) | 幅 |
|---|---|---|---|
| ダウンワードのみ(旧) | -52.7 dBFS | -13.1 dBFS | 39.5 dB |
| **+ パラレル(makeup +17 dB)** | **-35.3 dBFS** | -12.5 dBFS | **22.9 dB** |
| Waves MV2 (Low Level +14 dB) | -39.6 dBFS | -11.8 dBFS | 27.7 dB |

プラグインより ffmpeg のほうが大音量部への影響が小さかった。`--parallel 0` で無効。
ラベルを使うフィルタになるため、`master_chain` の戻り値には `;` が入る
(`-af` では扱えず `-filter_complex` が要る。`measure` は自動で切り替える)。

### ホール残響

実在ホールで実測したインパルス応答を **15%** 混ぜる。既定は Waves Complete IR Library の
**Birmingham Symphony Hall**(残響 2.00 秒)。世界有数のオーケストラ用ホールで、測った減衰も
公称値(1.85〜2.4 秒)と一致する。畳み込みは `pedalboard.Convolution`(pip)で行い、
**Waves のプラグイン本体は実行時に要らない**。`assets/ir/hall.wav` を置くとそちらが
優先される。`--reverb-ir` で別のホールを指定でき、`--reverb-mix 0` で無効。

ライブラリの `.wir` は独自形式に見えるが、**中身は RIFF WAVE のヘッダ識別子を2箇所
差し替えただけ**である。`RIFF`→`wvIR`、`WAVE`→`ver1`、加えて bits/sample に 23 という
無効な値が入っている。この3箇所を戻せば標準の 32bit float WAV として読める
(`reverb.read_wir`)。ファイル名の末尾が収録条件で、`m`=モノ / `x`=XYステレオ(2ch) /
`s`=トゥルーステレオ(4ch)、`o`=無指向 / `c`=単一指向。ここでは `x` の 2ch を使う。

畳み込みの前に IR へ二つ手を入れている。**直接音より前を捨てて先頭に 30 ms の無音を足す**
(実測 IR は直接音が 1 ms 付近から始まるので、そのまま混ぜると残響が音に張り付いて輪郭が鈍る。
以前使っていた IRLive の `Hall 2` は 37 ms のプリディレイを持っていた)。それと
**エネルギーを 1 に正規化する**(ホールごとにピークが 0.2〜4476 とばらばらで、
そのままでは `--reverb-mix` の意味がホールごとに変わってしまう)。この正規化のあとで
測ると、`Hall 2` 時代との wet レベルの差は **0.64 dB** に収まっている。

IR ライブラリは Waves のライセンス品なので、**このリポジトリには同梱していない**。
入っていない環境では `assets/ir/hall.wav` に落ちる。これは `tools/make_fallback_ir.py` が
作る**合成の IR** で、どこかの実在ホールを模したものではない。初期反射6発と、帯域ごとに
減衰時間の違う拡散残響(中低域 2.1 秒 → 10 kHz 付近 0.8 秒)を重ねたもので、実測すると
残響 1.72 秒になる。同梱 IR は代替であって上書きではないので、実測ホールがあればそちらが勝つ。

残響は**マスターチェーンの手前**に置く。あとにすると足した残響のぶんラウドネスと
トゥルーピークがずれて、ゲイン収束の保証が崩れる。出力は元と同じ長さで打ち切る
(尺が変わると確定境界やダイジェストの区間と食い違うため)。

ffmpeg の `afir` は使えなかった。24bit の IR では `Invalid argument` で落ち、float32 に
直しても wet 側が出力されない。Waves の `.wir` も独自形式で中身は素の PCM ではない。
...
| `mix` | `--parallel` | 17 dB | パラレルコンプの makeup。0 で無効 |
| `mix` | `--reverb-mix` | 0.15 | ホール残響を混ぜる割合。0 で無効 |
| `mix` | `--reverb-ir` | IRLive の Hall 3 | インパルス応答の WAV |
