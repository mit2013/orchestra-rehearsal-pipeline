...
特定の日だけ変えたいときはそちらを使う。`mix` と `field-export` の両方が同じ値を読む。

この録音環境(ZOOM F3 + 外部マイク、暗騒音 -70 dBFS 前後)では残響 0.15 と
パラレルコンプ +17 dB を採用しているが、**それはこの環境での結論であって既定値ではない。**
条件の異なる環境では、まず 0 から始めて、`mix` が毎回出す暗騒音の診断を見ながら
足していくのがよい。
...
**パラレルコンプの抑制も `mix` と揃えてある。** 以前は `field-export` が持ち上げ量を
渡さずにマスターチェーンを組み立てていたため、既定の +17 dB が黙って使われ、
`limit_parallel_makeup` を一度も通らなかった。空調の大きい会場(260905)の速報版で、
指揮者が話す場面の「サー」が目立つ原因になっている。いまはブロックごとに暗騒音を
測って持ち上げ量を決める。プロキシは1本に全ブロックが入っているので、測定は
