...
会場によっては空調が大きい。260905(管セクション練習・狭い部屋・低い天井・天井埋め込み式の
エアコンがフルパワー)がそれで、指揮者が話す場面で「サー」がはっきり聞こえた。**パラレルコンプは
暗騒音も一緒に持ち上げる**ので、聴き手には「ここから音をいじったな」と分かってしまう。
持ち上げ量を下げれば目立たなくなるが、今度は指揮者の声が聞き取れない。

そこで、持ち上げる前に引く。`--denoise 12` で有効になる(既定 0 = 無効)。実測したノイズ形状での
スペクトル減算で、静かなフレーム(エネルギー下位 10%)の平均パワースペクトルを引く。260905 の
3ブロックでの実測は次のとおり。
...
ffmpeg の `afftdn` は使えなかった。実測で最大 0.6 dB しか減らない。`residual_floor` の既定が
-38 dB で暗騒音が -36 dBFS にいるとほぼ頭打ちになること、`nt=white` が白色雑音を仮定するのに
対してこの空調ノイズは**エネルギーの 64% が 160〜500 Hz に寄っている**ことの2点による。
形の合わないモデルを当てていた。

引きすぎると、残った成分が単独の正弦波のように鳴る(musical noise)。ゲインに
`10^(-reduce_db/20)` の下限を置いてそれを抑えており、これが `--denoise` の値の実体である。
-12 dB は「『無音』の時だけ人工的なシュワシュワがちょっと気になるかな、程度」という
評価で選んだ。順序は**残響のあと、マスターチェーンの前**。詳細は `orchpipe/denoise.py`。

なお、マイクを音源に近づけるほうが本質的な対策である(距離が半分で +6 dB)。ハイパスフィルタは
効かない。250 Hz 以下を全部落としても暗騒音は 4.1 dB しか減らず、そのぶん管の基音が痩せる。
吹き込みではないので風防も関係しない。

### 耳で決める処理は既定で無効

残響・パラレルコンプ・ノイズ除去の3つは、**組み込みの既定がすべて 0(無効)**である。
効果を数値で検証できる処理(ラウドネス、トゥルーピーク、コンプ)とは扱いを分けている。

理由は、この3つが**会場と機材で適・不適が変わり、しかもソフトが自分の出来を判定できない**
ことにある。残響が合うかどうかも、静音部をどれだけ持ち上げてよいかも、その部屋を聴いた人に
しか分からない。知らない環境に強い加工を既定で当てると、使う人は「何かおかしいが原因が
分からない」という状態に置かれる。何も掛からないほうがまだ安全である。

自分の会場で値が決まったら、プロジェクト直下の `pipeline_defaults.json` に書く。
このファイルは各自の環境の置き場で、リポジトリには入らない(`.gitignore` 済み)。
...
コマンドラインの `--reverb-mix` / `--parallel` / `--denoise` は、指定すればこれより優先される。
特定の日だけ変えたいときはそちらを使う。`mix` はこの3つすべてを設定ファイルから読むが、
**`field-export`(速報版)は `parallel_db` だけ読まない**。理由は前節のとおり。

この録音環境(ZOOM F3 + 外部マイク)では残響 0.15 を常用している。パラレルコンプは
260802 で +17 dB を採用したが、その後は暗騒音の大きい会場が続き、**いまは 0(かけない)を
既定にして、かける日だけ耳で確かめて決めている**。静音部の中央値が -44 dBFS を超えると
「いじった音」と分かる、というのが4回の評価から見えている線である。条件の異なる環境では、
まず 0 から始めて、`mix` が毎回出す暗騒音の診断を見ながら足していくのがよい。
...
プロキシに載せるのは**クリップを避けるためのゲインだけ**である。
ラウドネス正規化・パラレルコンプ・コンプ・リミッターは、境界が決まったあとに母艦が
ブロックごとに当てる(`field-export`)。境界が決まる前に単一ゲインを確定させると、ブロック間の
音量差(260829 で 9.7 dB)がそのまま残ってしまうためである。このゲインは可逆なので、
母艦は測定の前に同じだけ戻す。
...
| 無音閾値 | -42.985 → -56.984 dBFS(当時の固定ゲイン -14 dB ぶんちょうど) |
| 音の差 | 位置ずれ 0.00 ms / 相関 0.9998 以上 / 残差は最悪 -34.6 dB(打楽器) |

### プロキシのゲインは、その日のピークから決める

**固定値にしてはいけない。** 以前は固定 -14 dB を当てていた。素材のピークが 0 dBFS を
超えることがあるためで(260829 で +11.43 dBFS)、MP3 に符号化する前に下げないと潰れる。
...
速報版にパラレルコンプを掛けたとき、その雑音ごと 18 dB 持ち上がり、聴いた人の評価は
「完全に音が壊れている感じ」だった。同じ +17 dB を原本から作ったほうは
「全く気にならない」である。

**形式の問題ではない。** 同じ素材を MP3 320k / MP3 V0 / AAC 256k / Opus 128k・192k で
作り比べても、静寂部の超過は +7 dB 前後で横並びだった。決め手は**可逆の FLAC でも
同じだけ出た**ことで、犯人は符号化ではなく置き場所の低さである。

そこで **符号化の前に 1 パス測り、ピークが -1 dBFS に来るゲインを当てる**。
260912 の実データで確かめた結果:

| | 1-2 kHz | 4-8 kHz | 8-16 kHz | 静寂部の S/N |
|---|---|---|---|---|
| 固定 -14 dB(従来) | +2.5 dB | +5.1 dB | +7.4 dB | 13.6 dB |
| 実測 +14.5 dB(現行) | **±0.0 dB** | **±0.0 dB** | **±0.0 dB** | **32.2 dB** |

パラレルコンプ +17 dB を通したあとの高域も、4-8 kHz で 6.9 dB、8-16 kHz で 8.4 dB
下がった。なお**静音部の中央値はどちらも -35.6 / -35.8 dBFS でほとんど同じ**である。
広帯域の 1 数値ではこの違いは見えない。

測定パスは復号だけなので軽い(母艦で 3.5 時間ぶんが約 10 秒)。以前は「microSD からの
読み出しが 2 回発生して律速する」ことを理由に避けていたが、実際の運用では WAV を先に
iPhone へコピーしてから流すので、その前提はもう当たらない。

**当てたゲインはファイル名で運ぶ。** 現場の出力は `{date}_proxy_gain{±X.X}dB.mp3` で、
`field-receive` がそれを読んで `ingest.json` の `field_proxy.gain_db` に控え、
`field-export` が測定の前に打ち消す。名前を変えるとゲインを取り違えるので変えないこと。
名前にゲインの無い古いプロキシは、従来どおり -14 dB とみなす。

### 速報版にパラレルコンプはかけない

`field-export` の `--parallel` の既定は **0(かけない)**で、`pipeline_defaults.json` の
`mastering.parallel_db` を**読まない**。ここだけ `mix` と既定が違う。

理由は、現場では人が細かく聴き比べる時間がないことにある。パラレルコンプは静音部を
持ち上げると同時に**暗騒音も持ち上げる**ので、空調の回る会場では「サー」が目立つ。
それに気づかないまま配ってしまうと、団員の手元に残るのはその版である。260905 と
260912 で実際にそうなった(当時は `field-export` が持ち上げ量を渡さずにチェーンを
組み立てていたため、既定の +17 dB が黙って使われ、`limit_parallel_makeup` を一度も
通らなかった)。

かけたい日は `--parallel 17` のように明示する。渡せば `mix` と同じくブロックごとに
暗騒音を測って持ち上げ量を抑える。プロキシは1本に全ブロックが入っているので、測定は
そのブロックの範囲だけを、固定ゲインを戻したうえで行う。

正式版(`mix`)でかけるかどうかは、**配布前に人が耳で決める**。同じ箇所を複数の設定で
並べた聴き比べを作って判断する運用にしている。
...
| `mix` | `--parallel` | 設定ファイル | パラレルコンプの makeup。0 で無効 |
| `mix` | `--reverb-mix` | 設定ファイル | ホール残響を混ぜる割合。0 で無効 |
| `mix` | `--reverb-ir` | IRLive の Hall 3 | インパルス応答の WAV |
| `mix` | `--denoise` | 設定ファイル | 空調などの定常音を実測形状で引く量 [dB]。入れるなら 12 前後 |
...
| `field-script` / `field-proxy` | `--gain` | **測って決める** | 固定したいときだけ指定する |
| `field-export` | `--gain` | `ingest.json` の値 | プロキシに載っていたゲイン。測定前に打ち消す |
...
| `field-export` | `--parallel` | **0(かけない)** | 速報版だけは設定ファイルを読まない。かける日は明示する |
| `field-export` | `--noise-ceiling` | mix と同じ | 暗騒音の天井 |
