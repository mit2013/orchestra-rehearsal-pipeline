...
# チューニング開始の手前に置く余裕。**0 にしてある。**
#
# 当初は 5 秒だった。検出開始が実測で最大 0.5 秒ほど遅れうるので、その手前から
# 始めておけば最初の A を欠かない、という理屈である。ところが 260829 の境界を
# レビューページで聴いてもらったところ、3ブロックとも「頭を +5 秒(= 検出位置
# ちょうど)に詰めてよい」という判定だった。実測でも余裕は足りている:
#
#   ブロック  検出位置での A 成分比率   +0.5 秒   +1.0 秒
#   前半      0.105                    0.330     0.565
#   中盤      0.034                    0.675     ―
#
# **検出位置では A はまだ立っていない。** tuning.py は核となる区間を見つけたあと
# 倍音で頭を前へ伸ばす(`tuning_head_mask`)ので、報告される開始は実際に音が
# 出るより手前に来る。つまり余裕は検出側で既に取られており、ここで重ねる必要がない。
# 5 秒のままだと、その前の音出しと無音がブロックの頭に入るだけになる。
TUNING_PRE_ROLL_S = 0.0
